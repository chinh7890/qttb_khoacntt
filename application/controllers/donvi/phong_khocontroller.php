...
class phong_khocontroller extends My_controller
{
	...
	public function them1phongkho()
	{
		...
		// lấy thông tin đơn vị
		$dv = $this->donvimodel->get_info($donvi);
		$idhocki = $this->session->userdata('idhocky');
		$check = $this->phong_khomodel->check_exists(array('maphong' => $maphong));
		if (!$check) {
			$data = array(
				'maphong' => $maphong,
				'tenphong' => $maphong . " " . $dv->tenviettat,
			...
			$this->phong_khomodel->themphongkho($data);
			//code mới
			$data_nk = array(
				'maphong' => $maphong,
				'magvql' => $magv,
			...
			$this->nhatkymodel->themlichsuqlphong($data_nk);
